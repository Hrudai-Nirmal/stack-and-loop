...
function AuditVisual() {
  return (
    <>
      ...
      <circle cx="88" cy="74" r="5" fill="var(--accent-2)" />
...
function AIVisual() {
  return (
    <>
      ...
      <path d="M224 69H260M224 86H252" stroke="var(--accent-2)" strokeLinecap="round" />
      <path d="M174 94L181 80L188 94L202 101L188 108L181 122L174 108L160 101Z" fill="none" stroke="var(--accent-2)" strokeLinejoin="round" />
    </>
  );
}
    ...
    <>
      ...
      {nodes.map(([cx, cy], index) => (
        <g key={`${cx}-${cy}`}>
          ...
          <circle cx={cx} cy={cy} r="3.5" fill={index === 1 ? "var(--accent-2)" : "currentColor"} />
        </g>
      ))}
    </>
  );
...
function MaintenanceVisual() {
  return (
    <>
      ...
      <circle cx="194" cy="63" r="5" fill="var(--accent-2)" />
