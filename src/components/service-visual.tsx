...
export function ServiceVisual({ variant }: { variant: ServiceVisualKey }) {
  return (
    <div className="relative h-40 overflow-hidden rounded-lg border hairline bg-black/18">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_20%,rgba(83,109,255,0.16),transparent_58%)]" />
      <svg
        viewBox="0 0 320 160"
...
function AuditVisual() {
  return (
    <>
      ...
      <rect x="56" y="52" width="64" height="44" rx="6" fill="rgba(83,109,255,0.14)" stroke="currentColor" />
      <rect x="172" y="86" width="48" height="28" rx="5" fill="rgba(255,255,255,0.04)" stroke="rgba(255,255,255,0.28)" />
      <path className="service-sketch-line" d="M54 128C102 96 142 118 184 70C214 36 246 42 274 56" fill="none" stroke="currentColor" strokeLinecap="round" strokeWidth="1.5" />
...
function AIVisual() {
  return (
    <>
      ...
      <path className="trace-fast" d="M124 80C156 80 158 52 188 52C218 52 218 80 248 80" fill="none" stroke="currentColor" strokeLinecap="round" strokeWidth="1.6" />
      <rect x="206" y="48" width="72" height="64" rx="8" fill="rgba(83,109,255,0.13)" stroke="currentColor" />
...
function IntegrationsVisual() {
  ...
  return (
    <>
      ...
      <path className="service-sketch-line" d="M72 52L160 38L246 62L220 112L110 112Z" fill="none" stroke="currentColor" strokeLinecap="round" strokeWidth="1.5" />
      {nodes.map(([cx, cy], index) => (
        <g key={`${cx}-${cy}`}>
          <circle cx={cx} cy={cy} r={index === 1 ? 18 : 14} fill={index === 1 ? "rgba(83,109,255,0.17)" : "rgba(255,255,255,0.04)"} stroke={index === 1 ? "currentColor" : "rgba(255,255,255,0.28)"} />
...
function MaintenanceVisual() {
  return (
    <>
      ...
      <circle cx="194" cy="63" r="13" fill="rgba(83,109,255,0.2)" />
