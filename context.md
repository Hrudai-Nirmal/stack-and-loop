...
- AI beam interaction: the box starts with a Human operator node in the center and 10 surrounding task nodes connected by bidirectional beams. The top-right `Automate` button fades into an `Automating workflow` transition with the progressive steps `Gathering the Pieces`, `Figuring out the Plan`, and `Making the Move`, then shows the automated AI workflow. In automated mode, five task boxes rotate through six tasks each. Clicking `Automated` returns to manual mode immediately without a transition.
