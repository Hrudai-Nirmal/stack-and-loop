...
- AI beam interaction: the box starts with a Human operator node in the center and 10 surrounding task nodes connected by bidirectional beams. The top-right `Automate` button crossfades this manual map into the AI workflow presentation and changes its label to `Automated`.
